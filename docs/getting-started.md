...
This guide shows you how to use the `geozarr-examples` library to create and validate GeoZarr-compliant metadata.
...
# or with uv
uv pip install "geozarr-examples @ git+https://github.com/developmentseed/geozarr-examples.git"
```
...
from geozarr_examples import validate_spatial, validate_proj
...
```python
import zarr
from geozarr_examples import validate_group, detect_conventions
...
```python exec="on" source="above" result="json"
from geozarr_examples import Spatial, SpatialConventionMetadata
...
```python exec="on" source="above" result="json"
from geozarr_examples import Proj, ProjConventionMetadata
